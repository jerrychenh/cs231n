...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    layer_type = bn_param.get('layer_type', 0)
...
#         cache = {'mean':sample_mean, 'var':sample_var, 'x':x, 'z':z, 'out':out, 'std': std, 'gamma':gamma}
        ...
        xbn = xmu * istd
        out = gamma * xbn + beta
        cache = (x, mu, xmu, sq, var, std, istd, xbn, eps, gamma, layer_type)
        
        if layer_type == 0: # 0 for batch norm
...
def batchnorm_backward(dout, cache):
    ...
    # u = sum(x)/N            (D,)
    # v = sum((x - u)**2) / N    (D,)
    x, mu, xmu, sq, var, std, istd, xbn, eps, gamma, layer_type = cache
...
#     out = gamma * xbn + beta        (N,D)
    ...
    dbeta = np.sum(dout, axis = layer_type)
    dgamma = np.sum(dout * xbn , axis = layer_type)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    Hq = (int) (1 + (H + 2 * pad - HH) / stride)
    Wq = (int) (1 + (W + 2 * pad - WW) / stride)
    out = np.zeros((N, F, Hq, Wq))
        ...
        for f in range(F):
            for i in range(Hq):
                for j in range(Wq):
                    ...
                    jstart = j * stride
                    jend = jstart + WW
                    out[n,f,i,j] = np.sum(x_pad[n,:,istart:iend,jstart:jend] * w[f,:,:,:]) + b[f]
        ...
        for f in range(F):
            for i in range(Hq):
                for j in range(Wq):
                    ...
                    jstart = j * stride
                    jend = jstart + WW
                    dw[f, :, :, :] += dout[n,f,i,j] * x_pad[n,:,istart:iend,jstart:jend]
                    dx_pad[n,:,istart:iend,jstart:jend] += dout[n,f,i,j] * w[f, :, :, :]
...
def max_pool_forward_naive(x, pool_param):
    ...
    Hq = (int)(1 + (H - pool_height)/stride)
    Wq = (int)(1 + (W - pool_width)/stride)
    
    out = np.zeros((N, C, Hq, Wq))
    
    for i in range(Hq):
        for j in range(Wq):
            istart = i * stride
            iend = istart + pool_height
            jstart = j * stride
            jend = jstart + pool_width
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(Hq):
        for j in range(Wq):
            ...
            max_area = x[:,:,istart:iend, jstart:jend]
            max_pos= np.max(x[:,:,istart:iend, jstart:jend], axis=(2,3))
            # the None,None helps the broadcast
            mask = max_area == max_pos[:,:,None,None]
            #print(mask * dout[:,:,i,j][:,:,None,None])
            dx[:,:,istart:iend, jstart:jend] = mask * dout[:,:,i,j][:,:,None,None]
